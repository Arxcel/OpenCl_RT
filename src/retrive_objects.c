...
/*   Updated: 2018/03/23 15:40:14 by vkozlov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		set_t_type(json_value *value)
{
	...
	if (value->type != json_string)
		err = 1;
	if (!err && !ft_strcmp(value->u.string.ptr, ""))
		val = T_STD;
	else if (!err && !ft_strcmp(value->u.string.ptr, "chessboard"))
		val = T_CHECK;
	...
	char *n, struct _json_value *val)
{
	*o = !ft_strcmp(n, "type") ? set_defaults(val) : *o;
	...
	o->tex_angle = !ft_strcmp(n, "tex_angle") ? get_number(val) : o->tex_angle;
}
