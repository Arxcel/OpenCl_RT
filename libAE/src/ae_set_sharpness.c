...
/*   Updated: 2018/03/22 13:11:22 by afarapon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lib_ae.h"

static unsigned int			get_c_red(size_t x,
...
static unsigned int			get_c_green(size_t x,
	size_t y, unsigned int *in, size_t w)
{
	t_color		result;
...
static unsigned int			get_c_blue(size_t x,
	size_t y, unsigned int *in, size_t w)
{
	t_color		result;
...
unsigned int				set_sharpness(t_ae *ae,
	size_t x, size_t y, unsigned int *in)
{
	...
	w = *ae->w;
	tmp.argb.red = clamp_rgb(get_c_red(x, y, in, *ae->w));
	tmp.argb.green = clamp_rgb(get_c_green(x, y, in, *ae->w));
	tmp.argb.blue = clamp_rgb(get_c_blue(x, y, in, *ae->w));
	return (tmp.col);
}
